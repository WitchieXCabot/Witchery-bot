...
config: {
    name: 'herbfind',
    category: "divination",
    usage: "wherbfind apple",
    aliases: ['hfind', 'find', 'herb'],
    description: "herb search command",
},
run: async (bot, message, args) => {
    if (message.guild.id !== '752356763268939876' && message.guild.id !== '790834414095040543' && message.guild.id !== '793162542729134120') return message.reply('This command can only be used in `2` server/s.')
        ...
        .setTitle('ERR!')
        .setDescription('\n`\`\`diff\n-WARNING: You didn\'t provide any herb/plant/flower! or we dont have that added! Request a herb, tree, plant or fruit for devs to add using Wsuggest <suggestion> (please state/mention Herb Find command in suggestion) \`\`\` `\`\`diff\n+use `Wherbfind list` for a full list of herbs\`\`\`')
        .setColor(color)
